...
function isCreditCardNumber(ccn) {
  ...
  arr.forEach((item, index) => {
    if (index % 2 === 0) {
      arr[index] = item * 2 > 9 ? item * 2 - 9 : item * 2;
    } else {
      arr[index] = item * 1;
    }
  });
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (close.includes(str[i])) {
      ...
      if (open.indexOf(stack[stack.length - 1]) === close.indexOf(str[i])) stack.pop();
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let h = 0; h < pathes.length; h += 1) {
    arr.push(pathes[h].split('/'));
  }

  for (let i = 0; i < arr[0].length - 1; i += 1) {
    if (arr[0][i] === arr[1][i] && arr.length === 2) {
      res.push(`${arr[0][i]}/`);
    }
    if (arr.length === 3) {
